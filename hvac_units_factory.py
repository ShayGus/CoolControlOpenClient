        ...
        for id, unit in units.data.items():                       
            if unit["type"] == 1:
                hvac_unit = HVACUnit(
                    ...
                    supported_operation_modes=[self._client.operation_modes.get(mode) for mode in unit["supportedOperationModes"]],
                    supported_operation_statuses=[self._client.operation_statuses.get(status) for status in unit["supportedOperationStatuses"]],
